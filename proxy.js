...
exports.getOnlineUser = function(cookie, callback) {
  var onlineoptions = {
    host: "172.18.1.48",
    port: "80",
    method: "GET",
    path: "/seeyon/message.do?method=showOnlineUserList&condition=SelectAccount",
    headers: {
      "Pragma": "no - cache",
      "Cache - Control": "no - cache",
      "Connection": "keep-alive",
      ...
      "Accept-Encoding": "gzip, deflate, sdch",
      "Accept-Language": "zh-CN,zh;q=0.8",
      "Referer": "http://172.18.1.48/seeyon/message.do?method=showOnlineUserTree",
      "Cookie": cookie
    }
  };
  http.get(onlineoptions, function(res) {
    var chunks = [],
      size = 0;
    res.on('data', function(chunk) {
      chunks.push(chunk);
    ...
    res.on('end', function() {
      ...
      zlib.unzip(data, function(err, buffer) {
        if (!err) {
          var html = buffer.toString();
          processData.getonlineuser(html, callback);
        }
      });
    })
  });
